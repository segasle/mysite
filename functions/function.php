...
function post_blog(){
    $sql = do_query('SELECT * FROM `maps_blog` WHERE 1');
    $out = '';
    ...
    while ($rs = mysqli_fetch_array($sql)){
        ...
        $out .= '<div class="maps">
                    <div class="maps_container">
                        <div class="maps_head">
<img src="'.$rs['img'].'" alt="">                            
                                <div class="maps_head_title">
                                    <a href="'.$rs['url'].'">'.$rs['header'].'</a>
                                </div>
                            </div>
                        </div>
                        <div class="maps_title">
                            <p>'.$rs['title'].'</p>
                        </div>  <div class="maps_data">
                            <p>'.$mydata->format('d.m.Y, H:m').'</p>
                         </div>
                        <div class="author">
                            <div class="author_img"><img src="'.$rs['author'].'" alt=""></div>
                            <div class="author_fio">'.$rs['fio'].'</div>
                        </div>
